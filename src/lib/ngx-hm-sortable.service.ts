...
export class NgxHmSortableService {
  ...
  mark(elm: HTMLElement, type: 1 | -1, className: string) {
    this.cleanMark();
    if (className) {
      this._renderer.addClass(elm, className);
      this._renderer.setAttribute(elm, 'add-class', className);
    } else {
      this._renderer.setStyle(elm, 'background-color', 'yellow');
    }
  ...
  private cleanMark() {
    if (this.prevSelector) {
      const className = this.prevSelector.attributes.getNamedItem('add-class');
      if (className) {
        this._renderer.removeClass(this.prevSelector, className.value);
      } else {
        this._renderer.setStyle(this.prevSelector, 'background-color', '');
      }
